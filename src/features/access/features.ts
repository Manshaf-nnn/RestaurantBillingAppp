...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    group: 'Accounting',
    description: 'The money identities, per-bill payment checks, bank matching and the issues list.',
    actions: [
      { key: 'view', permission: PERMISSIONS.ACCOUNTING_VIEW },
      {
        key: 'edit',
        label: 'Notes & acknowledgements',
        permission: PERMISSIONS.ACCOUNTING_NOTE,
        hint: 'Signed, append-only remarks on financial records.',
      },
      {
        key: 'approve',
        label: 'Bank matching',
        permission: PERMISSIONS.ACCOUNTING_RECONCILE,
        hint: 'Uploading statements and accepting matches moves no money.',
      },
    ],
    routes: ['/dashboard/accounting/reconciliation'],
  },
  {
    key: 'accountingMonthClose',
    label: 'Close month',
    group: 'Accounting',
    description: 'The month-end checklist, and sealing the month once it is clear.',
    actions: [
      { key: 'view', permission: PERMISSIONS.ACCOUNTING_VIEW },
      { key: 'approve', label: 'Close & reopen', permission: PERMISSIONS.ACCOUNTING_CLOSE },
    ],
    routes: ['/dashboard/accounting/close'],
  },
  {
    key: 'accountingLedger',
    label: 'Ledger',
    group: 'Accounting',
    description: 'Journal, trial balance, cash book and financial position — derived, read-only.',
    actions: [{ key: 'view', permission: PERMISSIONS.ACCOUNTING_VIEW }],
    routes: ['/dashboard/accounting/ledger'],
  },
  {
    key: 'accountingReports',
    label: 'Accounting reports',
    group: 'Accounting',
    description: 'The P&L, menu profitability, period comparison and every other report.',
    actions: [{ key: 'view', permission: PERMISSIONS.ACCOUNTING_VIEW }],
    routes: ['/dashboard/accounting/reports', '/dashboard/accounting/reports/variance'],
  },
  {
    key: 'accountingTools',
    label: 'Accounting tools',
    group: 'Accounting',
    description: 'The calculator and what-if simulator — nothing here writes a record.',
    actions: [{ key: 'view', permission: PERMISSIONS.ACCOUNTING_VIEW }],
    routes: ['/dashboard/accounting/tools'],
  },
  {
    key: 'accountingClose',
